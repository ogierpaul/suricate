...
def _df_to_dump(df, pkey):
    ...
    Args:
        df (pd.DataFrame): with 1-column index
        pkey (str): name of index
...
def es_index(client, df, index, doc_type, index_id=None, sleep=1):
    """

    Args:
        client (elasticsearch.Elasticsearch): elastic search client
        df (pd.DataFrame): pd.DataFrame, with 1-column index
        index (str): name of es index
        doc_type (str): name of document type
        index_id (str): name of index id
        sleep (int): number of seconds to wait after to complete indexation

    Returns:
        None
    """
    dump = _df_to_dump(df=df, pkey=index_id)
    if index_id is None:
        for d in dump:
...
def es_create_load(df, client, drop, create, index, doc_type, index_id, mapping):
    """
    ...
    Args:
        ...
        index_id (str): String, name of column to be used as id for the indexation
    ...
    es_index(client=client, df=df, index=index, doc_type=doc_type, index_id=index_id, sleep=2)
    return None
